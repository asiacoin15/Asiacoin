...
#if defined(NDEBUG)
# error "Asiacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x33cc5dafe301f0d08c17a726357f0178530d6b94ea261d5d4cf12eeb0e2f9e19");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Asiacoin: starting difficulty is 1 / 2^12
...
{
    // Asiacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Asiacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Asiacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1455454206, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d1331bc0813eea97a336206c5c54889a837a13b7c88696c89620e5daeef8de64f4280ac13909df03abb872877fd82a38abaa4fc3ca286fba6ca28c0ac7bbb558") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1455454206;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4413194;
        ...
        assert(block.hashMerkleRoot == uint256("0xa3c9348f64e9c920321b2ef1579e71b2282e4974d8def706682c116a0975b3b9"));
...
unsigned char pchMessageStart[4] = { 0xed, 0x4f, 0x25, 0x83 }; // Asiacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AsiacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AsiacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AsiacoinMiner(CWallet *pwallet)
{
    printf("AsiacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("asiacoin-miner");
    ...
    try { loop {
        ...
        printf("Running AsiacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AsiacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AsiacoinMiner, pwallet));
}
